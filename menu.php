<?php
    ...
    $sqlcat = "SELECT DISTINCT tipo FROM menu";
        ...
        <div class="row">
            <div class="col-sm-3">
                <div class="list-group">
                    ...
                    <?php
                        $sql = 'SELECT id,name FROM menu WHERE tipo="'.$category.'" order by id, tipo ASC';
                        ...
                        if ($result->num_rows > 1) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '" href="?id='.$row['id'].'">'.$row['name'].'</a>';
                            }
                ...
                <nav aria-label="...">
                    <ul class="pagination justify-content-center">
                        <li class="page-item <?php if($id == 1) {echo 'disabled';} ?>">
                            <a class="page-link text-<?php if($id != 1) echo theme($theme); ?>" href="?id=1" aria-label="Start">
                                <span aria-hidden="true">&laquo;</span>
                                <span class="sr-only"><?php echo $arraylang[$lang]['start']; ?></span>
                            </a>
                        </li>
                        <li class="page-item <?php if($id == 1) {echo 'disabled';} ?>">
                            <a class="page-link text-<?php if($id != 1) echo theme($theme); ?>" href="?id=<?php echo (!empty($id)) ? $id-1 : '#' ?>"><?php echo $arraylang[$lang]['previous']; ?></a>
                        </li>
                        <li class="page-item <?php if($id == $maxid) {echo 'disabled';} ?>">
                            <a class="page-link text-<?php if($id != $maxid) echo theme($theme); ?>" href="?id=<?php echo (!empty($id)) ? $id+1 : '#'?>"><?php echo $arraylang[$lang]['next']; ?></a>
                        </li>
                        <li class="page-item <?php if($id == $maxid) {echo 'disabled';} ?>">
                            <a class="page-link text-<?php if($id != $maxid) echo theme($theme); ?>" href="?id=<?php echo $maxid; ?>" aria-label="End">
